...
class EdgesPreparation:
    def __init__(self, graph, args, dict_image_class=None):
        self.args = args
...
def define_args(params):
    dict_param = {"weights_movie_movie": params[0], "weights_movie_class": params[1],
                  ...
                  'threshold': params[5], 'seen percentage': params[6], "dataset": param[7]}
    print(dict_param)
    ...
    parser.add_argument('--graph_percentage', default=0.01)
    if param[7] == 'awa2':
        import torch
...
def obj_func_grid(params):
    ...
    if args.data_name == 'our_imdb':
        ...
        weighted_graph = graph_maker.import_awa2_graph(weights)
...
if __name__ == '__main__':
    ...
    parameters = {
        ...
        "seen_percentage": [0.7, 0.8]
    }
    num = 0
    for dataset in parameters["data_name"]:
                        ...
                        for norma_type in parameters["norma_types"]:
                            for threshold in parameters["threshold"]:
                                for per in parameters["seen_percentage"]:
                                    param = np.array([w_m_m, w_m_c, e_type, dim, norma_type, threshold, per, dataset])
    ...
    plots_2measures_vs_parameter(dict_measures, parameters["seen_percentage"], 'seen Percentage', parameters['data_name'],
